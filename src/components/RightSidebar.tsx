    ...
    <aside className="flex w-72 shrink-0 flex-col gap-0 overflow-y-auto border-l border-slate-700 bg-slate-950 p-4">
      ...
      <CollapsibleSection title="Grid" defaultOpen>
        ...
        <div className="mt-4 space-y-2">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Transform</p>
          <button
            type="button"
            disabled={!hasImage}
          ...
          <Field label={`Width ${scaleX}%`}>
            <input
              type="range" min={10} max={200} value={scaleX} disabled={!hasImage}
          ...
          <Field label={`Height ${scaleY}%`}>
            <input
              type="range" min={10} max={200} value={scaleY} disabled={!hasImage || lockAspect}
              onPointerDown={() => onBeginGridScale()}
              onChange={(e) => {
                const v = Number(e.target.value);
                setScaleY(v);
        ...
        {/* Label positions */}
        <div className="mt-4 space-y-2">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Label positions</p>
          <div className="space-y-1">
            <span className="text-xs text-slate-400">Vertical lines</span>
            <VerticalLabelCross
              showTop={settings.verticalLabelTop} showBottom={settings.verticalLabelBottom}
          ...
          <div className="space-y-1">
            ...
            <HorizontalLabelCross
              showLeft={settings.horizontalLabelLeft} showRight={settings.horizontalLabelRight}
          ...
          <button type="button"
            onClick={() => patchAndRefreshLabels({ hiddenLabelKeys: {} })}
            className={secondaryBtn}>
            Restore deleted labels
          </button>
          <button type="button"
            onClick={() => patchAndRefreshLabels({ ...settings, customVerticalLabels: {}, customHorizontalLabels: {} })}
            className={secondaryBtn}>
            Rebuild all labels
          </button>
        </div>
        ...
        {/* Line appearance */}
        <div className="mt-4 space-y-3">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Line appearance</p>
          <Field label="Vertical color">
            <input type="color" value={settings.verticalLineColor}
          ...
          <Field label={`Vertical thickness (${settings.verticalLineThickness}px)`}>
            <input type="range" min={1} max={12} value={settings.verticalLineThickness}
              onChange={(e) => patch({ verticalLineThickness: Number(e.target.value) })}
              className="w-full accent-cyan-500" />
          </Field>
          <Field label="Horizontal color">
            <input type="color" value={settings.horizontalLineColor}
              onChange={(e) => patch({ horizontalLineColor: e.target.value })}
              className="h-9 w-full cursor-pointer rounded border border-slate-700 bg-slate-900" />
          </Field>
          <Field label={`Horizontal thickness (${settings.horizontalLineThickness}px)`}>
            <input type="range" min={1} max={12} value={settings.horizontalLineThickness}
              onChange={(e) => patch({ horizontalLineThickness: Number(e.target.value) })}
              className="w-full accent-cyan-500" />
          </Field>
          <Field label={`Opacity (${Math.round(settings.lineOpacity * 100)}%)`}>
            <input type="range" min={10} max={100} value={Math.round(settings.lineOpacity * 100)}
              onChange={(e) => patch({ lineOpacity: Number(e.target.value) / 100 })}
              className="w-full accent-cyan-500" />
          </Field>
          <Field label="Line style">
            <select value={settings.lineStyle}
              onChange={(e) => patch({ lineStyle: e.target.value as GridSettings["lineStyle"] })}
              className={selectClass}>
        ...
        {selectedLineId && (
          <div className="mt-4 space-y-2">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Selected line</p>
            ...
            <button type="button" onClick={onDeleteLine}
      ...
      <CollapsibleSection title="Labels" defaultOpen>
        ...
        <div className="mt-3 space-y-3">
          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Label appearance</p>
          <Field label={`Font size (${settings.labelFontSize}px)`}>
            <input type="range" min={10} max={32} value={settings.labelFontSize}
              onChange={(e) => patch({ labelFontSize: Number(e.target.value) })}
              className="w-full accent-cyan-500" />
          </Field>
          <Field label="Text color">
            <input type="color" value={settings.labelColor}
              onChange={(e) => patch({ labelColor: e.target.value })}
              className="h-9 w-full cursor-pointer rounded border border-slate-700 bg-slate-900" />
          </Field>
          <label className="mb-2 flex items-center gap-2 text-sm text-slate-300">
            <input type="checkbox" checked={settings.labelBackgroundEnabled}
              onChange={(e) => patch({ labelBackgroundEnabled: e.target.checked })}
              className="accent-cyan-500" />
            Label background
          </label>
          {settings.labelBackgroundEnabled && (
            <Field label="Background color">
              <input type="color"
                ...
                onChange={(e) => patch({ labelBackgroundColor: rgbaFromHex(e.target.value, 0.88) })}
                className="h-9 w-full cursor-pointer rounded border border-slate-700 bg-slate-900" />
            </Field>
          )}
          <button type="button" onClick={() => onRefreshLabelAppearance(settings)} className={secondaryBtn}>
            Apply label style
          </button>
        </div>
      </CollapsibleSection>
              ...
              {anchors.map((anchor) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 px-2 py-1.5">
                    <button type="button" onClick={() => onSelectAnchor(anchor.id)}
                    ...
                    <button type="button" onClick={() => onDeleteAnchor(anchor.id)}
                      className="text-slate-600 hover:text-red-400 text-xs px-1">✕</button>
                  </div>
                  {/* Notes field — visible when selected */}
                  {selectedAnchorId === anchor.id && (
                    <div className="px-2 pb-2">
                      <input
                        ...
                        placeholder="Notes (shown on canvas)…"
...
function CollapsibleSection({
  ...
  return (
    <div className="border-b border-slate-800 py-3">
      ...
      {open && <div className="mt-3 space-y-0">{children}</div>}
    </div>
  );
}
